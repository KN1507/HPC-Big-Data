...
void vectorAddCuda(int n, float* a, float* b, float* result) {
    ...
    cout << "vector-mul (kernel): \t\t" << kernelTime1  << endl;
    cout << "vector-mul (memory): \t\t" << memoryTime << endl;
}
...
int vectorAddSeq(int n, float* a, float* b, float* result) {
  ...
  cout << "vector-mul (sequential): \t\t" << sequentialTime << endl;
...
int main(int argc, char* argv[]) {
    ...
    cout << "Multiplying two vectors of " << n << " integer elements." << endl;

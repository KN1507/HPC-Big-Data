...
void vectorAddCuda(int n, float* a, float* b, float* result) {
    ...
    cout << "vector-sub (kernel): \t\t" << kernelTime1  << endl;
    cout << "vector-sub (memory): \t\t" << memoryTime << endl;
}
...
int vectorAddSeq(int n, float* a, float* b, float* result) {
  ...
  cout << "vector-sub (sequential): \t\t" << sequentialTime << endl;
...
int main(int argc, char* argv[]) {
    ...
    cout << "Subtracting two vectors of " << n << " integer elements." << endl;

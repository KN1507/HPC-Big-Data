...
__global__ void encryptKernel(int n,char* deviceDataIn,char* deviceDataOut)
{
    const int even_key = 1 ;
    const int odd_key = 2;
    int dummy;
    unsigned index = blockIdx.x * blockDim.x + threadIdx.x;
    deviceDataOut[index] = (index%2==0)? (deviceDataOut[index]=deviceDataIn[index] + even_key): (deviceDataOut[index]=deviceDataIn[index] + odd_key);

}
__global__ void decryptKernel(int n,char* deviceDataIn, char* deviceDataOut) {
    const int even_key = 1;
    const int odd_key = 2;
    unsigned index = blockIdx.x * blockDim.x + threadIdx.x;
    deviceDataOut[index] = (index%2==0)?(deviceDataOut[index]=deviceDataIn[index] - even_key):(deviceDataOut[index]=deviceDataIn[index] - odd_key);
...
{  
  ...
  {
  data_out[i] = (i%2==0)?(data_out[i]=data_in[i] + even_key):(data_out[i]=data_in[i] + odd_key);
  }
...
int EncryptCuda (int n, char* data_in, char* data_out) {
    ...
    // execute kernel
    kernelTime1.start();
    encryptKernel<<<n/threadBlockSize, threadBlockSize>>>(n,deviceDataIn, deviceDataOut);
...
int DecryptCuda (int n, char* data_in, char* data_out) {
    ...
    // execute kernel
    kernelTime1.start();
    decryptKernel<<<n/threadBlockSize, threadBlockSize>>>(n,deviceDataIn, deviceDataOut);
    cudaDeviceSynchronize();
    kernelTime1.stop();

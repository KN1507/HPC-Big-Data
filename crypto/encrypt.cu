...
__global__ void decryptKernel(int n,char* deviceDataIn, char* deviceDataOut) {
  ...
  if(index<n){  deviceDataOut[index] = deviceDataIn[index] - 1;
}}
...
int DecryptSeq (int n, char* data_in, char* data_out)
{
  ...
  // cout <<"comes to start timer"<<endl;
  sequentialTime.start();
  for(i=0; i<n ; i++)
  {
  	data_out[i] = data_in[i] - 1;
  ...
  //cout<< "comes out of for loop"<<endl;
  sequentialTime.stop();
  cout << fixed << setprecision(6);
  // cout <<"comes to return"<<endl;
  cout<< "Decryption: \t\t"<<sequentialTime.getElapsed()<<"seconds."<<endl;
  return 0;
}
